...
public class JavaApiSample implements Watcher {
	...
	private static final String ZK_PATH = "/hq";
	...
	public void createConnection(String connectString, int sessionTimeout) {
		...
		try {
			...
			connectedSemaphore.await(); //count!=0时，造成调用的线程阻塞
	...
	public boolean createPath(String path, String data) {
		try {
			System.out.println("节点创建成功, Path: " + this.zk.create(path, // path
					data.getBytes(), // data
					Ids.OPEN_ACL_UNSAFE, // 安全控制，一般设置为这个
					CreateMode.EPHEMERAL) + ", content: " + data); // 临时节点EPHEMERAL
		} catch (KeeperException e) {
			System.out.println("节点创建失败，发生KeeperException"); // KeeperException$NodeExistsException
			e.printStackTrace();
		} catch (InterruptedException e) {
	...
	public boolean writeData(String path, String data) {
		try {
			System.out.println("更新数据成功，path：" + path + ", stat: "
					+ this.zk.setData(path, data.getBytes(), -1)); // Stat包含版本信息，数据长度，事务ID等等。
		} catch (KeeperException e) {
	...
	public void process(WatchedEvent event) {
		...
		if (KeeperState.SyncConnected == event.getState()) {
			connectedSemaphore.countDown(); // 如果没有此句话，主线程就会一直阻塞，直到被Interrupt
		}
	}
	...
	public static void main(String[] args) {
		...
		if (sample.createPath(ZK_PATH, "Init我是节点初始内容")) {
			System.out.println();
			System.out.println("数据内容: " + sample.readData(ZK_PATH) + "\n");
			sample.writeData(ZK_PATH, "Update更新后的数据");
			System.out.println("数据内容: " + sample.readData(ZK_PATH) + "\n");
